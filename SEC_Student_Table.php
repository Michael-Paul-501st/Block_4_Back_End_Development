...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>SEC Student relationship</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $Student_SEC_Relation = $_GET["Student_SEC_Relation"];

        $sql_SEC_Student = "INSERT INTO `student_sec` (`Student_ID`, `SEC_ID`, `Student_SEC_Relation`) VALUES ('".$Student_ID."', '".$SEC_ID."', '".$Student_SEC_Relation."');";

        if ($conn->query($sql_SEC_Student) === TRUE) {//Add sec student relatshion to the record
            echo "New record created successfully";//return success messsage
        } 
        else {
            echo "Error: " . $sql_SEC_Student . "<br>" . $conn->error;//display error message
        }
