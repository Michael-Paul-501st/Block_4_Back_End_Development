...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Library book Student relationship</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $Student_ID =  $_GET["Student_ID"];//get information
        $Library_Book_ID = $_GET["Library_Book_ID"];

        $sql_student_library_book = "UPDATE Library_Book SET Student_ID = ".$Student_ID." WHERE Library_Book_ID = ".$Library_Book_ID.";";
        if ($conn->query($sql_student_library_book) === TRUE) {//add student id to library book
            echo "New record created successfully";//return success messsage
        } 
        else {
            echo "Error: " . $sql_student_library_book . "<br>" . $conn->error;//display error message
        }//add student id to library book
