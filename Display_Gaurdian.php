...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Display Gaurdian</title><!--The title of the web page -->
...
$sql_get_Gaurdian_info = "SELECT * FROM `Gaurdian` WHERE Gaurdian_ID = '". strval($Gaurdian_ID)."';";//display the gaurdian information
$Gaurdian_info = $conn->query($sql_get_Gaurdian_info);
if ($Gaurdian_info ->num_rows > 0){
    while ($row = $Gaurdian_info -> fetch_assoc()){
        $Gaurdian_Name =  $row["Gaurdian_Name"];
        $Gaurdian_Address =  $row["Gaurdian_Address"];
        $Gaurdian_Email =  $row["Gaurdian_Email"];
        $Gaurdian_Phone_number =  $row["Gaurdian_Phone_number"];
        echo "<div class='row'>";
    ...
    echo "No info for Gaurdian found";
}

$sql_get_student_ID = "SELECT * FROM `Student_Gaurdian` WHERE Gaurdian_ID = '". strval($Gaurdian_ID)."';";//get the students tied to the gaurdian information
...
if ($StudentID_result ->num_rows > 0){
    ...
    echo "<div class='row'>";
    echo "<div class='col'><h3>No student associated with Gaurdian</h3></div>";
    echo "</div>";
}
