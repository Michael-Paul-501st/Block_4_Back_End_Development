...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Gaurdian Student relationship</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $Gaurdian_ID =  $_GET["Gaurdian_ID"];
        $Student_ID =  $_GET["Student_ID"];
        $Student_Gaurdian_Relation = $_GET["Student_Gaurdian_Relation"];

        $sql_Gaurdian_Student = "INSERT INTO `student_gaurdian` (`Student_ID`, `Gaurdian_ID`, `Student_Gaurdian_Relation`) VALUES ('".$Student_ID."', '".$Gaurdian_ID."', '".$Student_Gaurdian_Relation."');";

        if ($conn->query($sql_Gaurdian_Student) === TRUE) {//Add Gaurdian student to the record
            echo "New record created successfully";//return success messsage
        } 
        else {
            echo "Error: " . $sql_Gaurdian_Student . "<br>" . $conn->error;//display error message
        }
