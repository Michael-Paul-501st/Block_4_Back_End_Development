...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Add Book</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $Library_Book_Name =  $_GET["Library_Book_Name"];
        $Library_Book_Reading_Age =  $_GET["Library_Book_Reading_Age"];

        $sql_book = "INSERT INTO `library_book` (`Library_Book_ID`, `Library_Book_Name`, `Library_Book_Reading_Age`, `Student_ID`) VALUES (NULL, '".$Library_Book_Name."', '".$Library_Book_Reading_Age."', NULL);";

        //echo "Add book";
        if ($conn->query($sql_book) === TRUE) {//Add book to the record
            echo "New record created successfully";//return success messsage
        } 
        else {
            echo "Error: " . $sql_book . "<br>" . $conn->error;//display error message
        }
