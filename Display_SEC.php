...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Display SEC</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $sql_get_SEC_info = "SELECT * FROM `Emergency_Contact` WHERE SEC_ID = '". strval($SEC_ID)."';";//get the info of the emergency contact and display it
        $SEC_info = $conn->query($sql_get_SEC_info);
        if ($SEC_info ->num_rows > 0){
            while ($row = $SEC_info -> fetch_assoc()){
            ...
            echo "No info for Emergency Contact found";
        }

        $sql_get_SEC_ID = "SELECT * FROM `Student_SEC` WHERE SEC_ID = '". strval($SEC_ID)."';";//get all student tied to the Emergencency contact and display it
        $StudentID_result = $conn->query($sql_get_SEC_ID);
        if ($StudentID_result ->num_rows > 0){
            while ($row = $StudentID_result -> fetch_assoc()){
                ...
                $sql_get_Student_info = "SELECT * FROM `Student` WHERE Student_ID = '". strval($Student_ID)."';";
                $studentID_result = $conn->query($sql_get_Student_info);
                if ($studentID_result ->num_rows > 0){
                    while ($row_1 = $studentID_result -> fetch_assoc()){
                    ...
                    echo "<div class='row'>";
                    echo "<div class='col'><h3>No student with that ID found</h3></div>";
                    echo "</div>";
                }
            }
        } 
            ...
            echo "<div class='row'>";
            echo "<div class='col'><h3>No student Info tied to the emrgeny contact</h3></div>";
            echo "</div>";
        }
