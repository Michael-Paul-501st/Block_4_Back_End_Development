...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Display TA</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $TA_ID =  $_GET["TA_ID"];//get TA ID

$sql_get_TA_info = "SELECT * FROM `Teacher_Assistant` WHERE TA_ID = '". strval($TA_ID)."';";//get the info for the TA and display it
$TA_info = $conn->query($sql_get_TA_info);
if ($TA_info ->num_rows > 0){
    while ($row = $TA_info -> fetch_assoc()){
        $TA_Name =  $row["TA_Name"];
        $TA_Address =  $row["TA_Address"];
        $TA_Email =  $row["TA_Email"];
        $TA_Safegaurding =  $row["TA_Safegaurding"];
        $TA_Phone_number =  $row["TA_Phone_number"];
        $TA_Annual_Salary =  $row["TA_Annual_Salary"];
        $TA_Background_check =  $row["TA_Background_Check"];
        echo "<div class='row'>";
        echo "<div class='col'><h3>Teaching Assistant Info</h3></div>";
        echo "</div>";
        echo "<div class='row'>";
    ...
    echo "<div class='row'>";
    echo "<div class='col'><h3>No Teaching Assistant Info</h3></div>";
    echo "</div>";
}

$sql_get_class_ID = "SELECT * FROM `TA_Class` WHERE TA_ID = '". strval($TA_ID)."';";//get the class info for the TA
...
if ($ClassID_result ->num_rows > 0){
    while ($row = $ClassID_result -> fetch_assoc()){
        ...
        if ($classID_result ->num_rows > 0){
            ...
            echo "<div class='row'>";
            echo "<div class='col'><h3>No class with ID Info found</h3></div>";
            echo "</div>";
        }
    }
} 
    ...
    echo "<div class='row'>";
    echo "<div class='col'><h3>No class tied to the teaching assistant</h3></div>";
    echo "</div>";
}
