...
<html lang="en"><!-- The create the html and set lang to english -->
    <head><!-- The head tag for html-->
        <title>Display Teacher</title><!--The title of the web page -->
    ...
    <main><!-- The main body of the website-->
        ...
        $Teacher_ID =  $_GET["Teacher_ID"];//get the teacehr ID

$sql_get_teacher_info = "SELECT * FROM `Teacher` WHERE Teacher_ID = '". strval($Teacher_ID)."';";//get and display the teacher informationn
...
if ($Teacher_info ->num_rows > 0){
    while ($row = $Teacher_info -> fetch_assoc()){
        ...
        $Teacher_Safegaurding =  $row["Teacher_Safegaurding"];
    ...
    echo "No info for Teacher found";
}

$sql_get_class_ID = "SELECT * FROM `Teacher_Class` WHERE Teacher_ID = '". strval($Teacher_ID)."';";//get the class tied to the teacher
...
if ($ClassID_result ->num_rows > 0){
    while ($row = $ClassID_result -> fetch_assoc()){
        ...
        if ($classID_result ->num_rows > 0){
            ...
            echo "<div class='row'>";
            echo "<div class='col'><h3>No class with that ID found</h3></div>";
            echo "</div>";
        }
    }
} 
    ...
    echo "<div class='row'>";
    echo "<div class='col'><h3>No class found</h3></div>";
    echo "</div>";
}
